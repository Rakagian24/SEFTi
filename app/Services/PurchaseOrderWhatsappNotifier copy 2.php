...
class PurchaseOrderWhatsappNotifier
{
    ...
    protected function sendTemplateMessage(string $phone, array $variables): void
    {
        ...
        // New simple template: only {{1}} is used and should always be the system name (e.g. SEFTi)
        $systemName = config('app.name', 'SEFTi');
        $templateVariables = ['1' => $systemName];

        $payload = [
            'from' => $fromWhatsapp,
            'contentSid' => $templateSid,
            'contentVariables' => json_encode($templateVariables, JSON_THROW_ON_ERROR),
        ];
