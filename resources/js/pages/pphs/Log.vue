...
import { User, Activity, Plus, Edit, Trash2, ArrowRight, ArrowLeft, FileText } from "lucide-vue-next";
...
const breadcrumbs = [
  ...
  { label: "PPh", href: "/pphs" },
  { label: `${props.pph?.kode_pph} - Log Activity` }
];
    ...
    <div class="pl-2 pt-6 pr-6 pb-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-lg font-semibold text-gray-900">{{ pph?.kode_pph }} - {{ pph?.nama_pph }}</h3>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-b-lg shadow-sm border border-gray-200 p-6">
        <div class="space-y-6">
          ...
          <div v-if="!logs?.data || logs.data.length === 0" class="text-center py-12">
            ...
            <p class="text-gray-500">There are no activities recorded for this PPh.</p>
          </div>
        </div>
        ...
        <button
            @click="goBack"
            class="flex items-center w-1/3 gap-2 px-3 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 hover:bg-white/50 rounded-md transition-colors duration-200"
          >
...
.dot-glow {
  box-shadow:
    ...
    0 0 16px 8px rgba(37, 99, 235, 0.20),
