    ...
    <div class="pl-2 pt-6 pr-6 pb-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Approval - Purchase Order Activities</h3>
      ...
      <div class="bg-white rounded-b-lg shadow-sm border border-gray-200 p-6">
        <div class="space-y-0">
          ...
          <div v-if="!logsList || logsList.length === 0" class="text-center py-12 col-span-3">
            ...
            <p class="text-gray-500">There are no activities recorded for this purchase order.</p>
          </div>
        </div>

        <!-- Pagination -->
        <div v-if="logsList && logsList.length > 0" class="mt-8 flex items-center justify-center border-t border-gray-200 pt-6">
          <nav class="flex items-center space-x-2" aria-label="Pagination">
            <button @click="prevPage" :disabled="!pagination.prev_page_url" :class="['px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200', pagination.prev_page_url ? 'text-gray-600 hover:text-gray-800 hover:bg-gray-50' : 'text-gray-400 cursor-not-allowed']">Previous</button>
            ...
            <button @click="nextPage" :disabled="!pagination.next_page_url" :class="['px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200', pagination.next_page_url ? 'text-gray-600 hover:text-gray-800 hover:bg-gray-50' : 'text-gray-400 cursor-not-allowed']">Next</button>
          </nav>
        </div>
      </div>
...
function prevPage() { handlePagination(pagination.value.prev_page_url ?? null); }
function nextPage() { handlePagination(pagination.value.next_page_url ?? null); }

</script>

<style scoped>
.timeline-item:hover { background-color: rgba(0, 0, 0, 0.02); }
