        ...
        <div class="px-4 pt-4 pb-6md:px-6 md:pt-6">
            ...
            <div class="mb-4 flex items-start justify-between gap-3 md:mb-6">
                <div class="flex items-center gap-4">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 md:text-2xl">Detail Purchase Order (Approval)</h1>
            ...
            <div v-if="purchaseOrder.status === 'Rejected' && purchaseOrder.rejection_reason" class="mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900">Informasi Purchase Order</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div class="space-y-4">
                                ...
                                <div v-if="purchaseOrder.tipe_po === 'Reguler'" class="flex items-start gap-3">
                                    <svg class="mt-0.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                ...
                                <div v-if="purchaseOrder.tipe_po === 'Lainnya'" class="flex items-start gap-3">
                                    <svg class="mt-0.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            <span class="ml-2 rounded-full bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700"
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th
                                                v-if="purchaseOrder?.perihal?.nama?.toLowerCase() === 'permintaan pembayaran uang saku'"
                                            ...
                                            <th class="px-6 py-4 text-center text-xs font-semibold tracking-wider text-gray-600 uppercase">Jumlah</th>
                                            <th class="px-6 py-4 text-center text-xs font-semibold tracking-wider text-gray-600 uppercase">Satuan</th>
                                            <th class="px-6 py-4 text-right text-xs font-semibold tracking-wider text-gray-600 uppercase">
                                                Harga Satuan
                                            </th>
                                            <th class="px-6 py-4 text-right text-xs font-semibold tracking-wider text-gray-600 uppercase">
                                                Subtotal
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        <tr v-for="(item, index) in purchaseOrder.items" :key="index" class="transition-colors hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="text-sm font-medium text-gray-900">{{ Number(index) + 1 }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="text-sm font-medium text-gray-900">{{ item.nama || item.nama_barang || '-' }}</span>
                                            </td>
                                            <td
                                                v-if="purchaseOrder?.perihal?.nama?.toLowerCase() === 'permintaan pembayaran uang saku'"
                                                class="px-6 py-4"
                                            >
                                                <div class="text-sm text-gray-900" v-if="item.bisnis_partner">
                                                    <div>
                                                        {{ item.bisnis_partner.nama_bp || item.bisnis_partner.nama_rekening || '-' }}
                                                    </div>
                                                </div>
                                            ...
                                            >
                                                <div class="text-sm text-gray-900" v-if="item.bisnis_partner">
                                                    <span class="font-mono">
                                                        {{ item.bisnis_partner.no_rekening_va || item.bisnis_partner.no_rekening || '-' }}
                                                    </span>
                                                </div>
                                                <span v-else class="text-sm text-gray-500">-</span>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <span class="text-sm font-medium text-gray-900">{{ formatCurrency(item.harga || 0) }}</span>
                                            </td>
                                            <td class="px-6 py-4 text-right">
                    ...
                    <TerminSummary v-if="purchaseOrder.tipe_po === 'Lainnya' && purchaseOrder.termin" :termin-data="purchaseOrder.termin" />
                  ...
                  <p class="text-sm text-gray-900 leading-relaxed">
                    {{
                      ...
                      "No additional requirements specified."
                            ...
                            <div>
                                <p class="mb-2 text-sm font-medium text-gray-900">Catatan</p>
                                <div class="rounded-lg bg-gray-50 p-4">
                                    <p class="text-sm leading-relaxed text-gray-900">
                                        {{ purchaseOrder.keterangan || purchaseOrder.note || 'No additional notes.' }}
                                    </p>
                                </div>
                            </div>

                            <div v-if="purchaseOrder.dokumen">
                                <p class="mb-2 text-sm font-medium text-gray-900">Dokumen Terlampir</p>
                ...
                <div class="space-y-6">
                    ...
                    <div v-if="purchaseOrder.metode_pembayaran" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        <div class="mb-4 flex items-center gap-2">
                            <svg class="h-5 w-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                                    stroke-linecap="round"
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <template v-if="purchaseOrder.metode_pembayaran === 'Transfer'">
                                ...
                                <!-- Default: Supplier / Bank Supplier Account -->
                                <template v-else>
                                    <div class="flex items-start gap-3">
                                        <svg class="mt-0.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-medium text-red-600">-{{ formatCurrency(diskonValue) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                            ...
                            <div v-if="purchaseOrder.dp_active && (purchaseOrder.dp_nominal || 0) > 0" class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-medium text-gray-900">
                                    ...
                                    <span
                                        v-if="purchaseOrder.dp_type === 'percent' && purchaseOrder.dp_percent != null"
              ...
              <div class="text-center">
                ...
                <div v-if="purchaseOrder.metode_pembayaran !== 'Kredit'" class="md:hidden">
                    <ApprovalProgress
                        :progress="approvalProgress"
                        :purchase-order="purchaseOrder"
                        :user-role="userRole"
            ...
            <div class="mt-6">
                <button
                    @click="goBack"
                    class="flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium text-gray-600 transition-colors duration-200 hover:bg-white/50 hover:text-gray-800"
                >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const subtotal = computed<number>(() => {
    ...
    if (items.length > 0) {
        return items.reduce((sum: number, item: any) => {
            ...
            const harga = Number(item?.harga ?? 0) || 0;
    ...
    if (po?.tipe_po === 'Lainnya') {
        return Number(po?.nominal ?? 0) || 0;
    }

    return Number(po?.harga ?? 0) || 0;
});

const diskonValue = computed<number>(() => {
    const po = purchaseOrder.value as any;
    return Number(po?.diskon ?? 0) || 0;
});

const dppValue = computed<number>(() => {
    const base = subtotal.value - diskonValue.value;
    return base > 0 ? base : 0;
});

const ppnValue = computed<number>(() => {
    const po = purchaseOrder.value as any;
    const ppnFlag = Boolean(po?.ppn);
    if (!ppnFlag) return 0;
    return dppValue.value * 0.11;
});

// DPP khusus PPh: hanya item bertipe Jasa; fallback Lainnya pakai nominal
const dppPphValue = computed<number>(() => {
    const po = purchaseOrder.value as any;
    const items = Array.isArray(po?.items) ? po.items : [];
...
const pphValue = computed<number>(() => {
    const po = purchaseOrder.value as any;
    const persen = Number(po?.pph?.tarif_pph ?? 0) || 0;
    if (!persen || !dppPphValue.value) return 0;
    return dppPphValue.value * (persen / 100);
});

const dpValue = computed<number>(() => {
    const po = purchaseOrder.value as any;
    if (!po?.dp_active) return 0;
    return Number(po?.dp_nominal ?? 0) || 0;
});
...
async function fetchApprovalProgress() {
    ...
    try {
        const response = await get(`/api/approval/purchase-orders/${purchaseOrder.value.id}/progress`);
...
const currentStep = computed(() => {
    return (approvalProgress.value || []).find((s: any) => s && s.status === 'current') || null;
});
...
const canVerify = computed(() => {
    ...
    if (departmentName.value === 'Zi&Glo' || departmentName.value === 'Human Greatness') return false; // Zi&Glo & Human Greatness have no verify step
    if (creatorRole.value === 'Staff Toko') return ['Kepala Toko', 'Admin'].includes(userRole.value);
    if (creatorRole.value === 'Staff Akunting & Finance') return ['Kabag', 'Admin'].includes(userRole.value);
    return false;
});
...
const canValidate = computed(() => {
    ...
    if (creatorRole.value === 'Staff Digital Marketing' || departmentName.value === 'Zi&Glo' || departmentName.value === 'Human Greatness') {
