...
const breadcrumbs = [
  ...
  { label: `${props.supplier?.nama_supplier} - Log Activity` }
];
...
function applyFilters() {
  ...
  router.get(`/suppliers/${props.supplier?.id}/logs`, params, {
    preserveState: true,
    preserveScroll: true,
  });
}
    ...
    <div class="pl-2 pt-6 pr-6 pb-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-lg font-semibold text-gray-900">{{ supplier?.nama_supplier }}</h3>
            <p class="text-sm text-gray-500">{{ supplier?.email }} • {{ supplier?.no_telepon }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-b-lg shadow-sm border border-gray-200 p-6">
        <div class="space-y-6">
          ...
          <div v-if="!logs?.data || logs.data.length === 0" class="text-center py-12">
            ...
            <p class="text-gray-500">There are no activities recorded for this supplier.</p>
          </div>
        </div>
