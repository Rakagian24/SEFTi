<template>
  <div class="bg-[#DFECF2] min-h-screen">
    <div class="px-4 pt-4 pb-6">
      ...
      <PageHeader title="Buat Realisasi" :show-add-button="false" />
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div v-show="activeTab === 'docs'">
          <RealisasiSupportingDocs ref="docsRef" :realisasi-id="draftId" />
        </div>
      </div>
...
async function doSaveDraft(showMessage = true): Promise<boolean> {
  ...
  try {
    let resp;
    if (draftId.value) {
      resp = await axios.patch(`/realisasi/${draftId.value}/update-draft`, formPayload, {
        withCredentials: true,
      });
    } else {
      resp = await axios.post('/realisasi/store-draft', formPayload, { withCredentials: true });
    }

    const id = resp?.data?.id;
    if (id) {
      draftId.value = id;
      // Sinkronkan checklist active & upload file setelah draft punya ID
...
async function doSend() {
  ...
  try {
    ...
    const response = await axios.post(
      '/realisasi/send',
      { ids: [draftId.value], documents_active: documentsActive },
      { withCredentials: true }
    );
