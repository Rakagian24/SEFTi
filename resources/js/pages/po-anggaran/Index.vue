  ...
  <div class="bg-[#DFECF2] min-h-screen">
    <div class="px-4 pt-4 pb-6">
      ...
      <div class="mb-6 hidden items-center justify-between md:flex">
        ...
        <div class="flex items-center gap-3">
          ...
          <ConfirmDialog
            ...
            :message="`Apakah Anda yakin ingin mengirim ${selected.length} PO Anggaran?`"
            @confirm="confirmSend"
            @cancel="cancelSend"
          />
      ...
      <div class="mb-4 md:hidden">
        <h1 class="text-xl font-bold text-gray-900">PO Anggaran</h1>
      ...
      <div class="hidden md:block">
        ...
        <PoAnggaranTable
          :data="poAnggarans?.data || []"
          :pagination="poAnggarans"
          :selected="selected"
          :columns="columns"
          @select="onSelect"
          @action="handleAction"
      ...
      <div class="mt-4 md:hidden">
        <!-- Simple search (binds to currentFilters) -->
        <div class="mb-4">
          <input
            ...
            placeholder="Cari No. PO Anggaran / Departemen / Perihal"
            @keyup.enter="loadPoAnggarans({ page: 1 })"
            @blur="loadPoAnggarans({ page: 1 })"
          />
        </div>

        <div
          v-if="poRows.length === 0"
          class="py-8 text-center text-sm text-gray-500"
        >
          Belum ada PO Anggaran yang terdaftar.
        </div>
        ...
        >
          ...
          >
            <button
              type="button"
              @click="handleMobilePaginate(poPage.prev_page_url as any)"
              :disabled="!poPage.prev_page_url"
              :class="[
                'rounded-full border px-3 py-1.5 font-medium transition-colors',
                poPage.prev_page_url
            ...
            <div class="px-3 py-1 text-[11px] text-gray-500">
              Halaman
              <span class="font-semibold text-gray-800">{{ (poPage.current_page as any) || 1 }}</span>
            </div>

            <button
              type="button"
              @click="handleMobilePaginate(poPage.next_page_url as any)"
              :disabled="!poPage.next_page_url"
              :class="[
                'rounded-full border px-3 py-1.5 font-medium transition-colors',
                poPage.next_page_url
                  ? 'border-gray-300 bg-white hover:bg-gray-50 hover:text-gray-800'
                  : 'border-transparent text-gray-400 cursor-not-allowed',
              ]"
            >
...
const currentFilters = ref(props.filters || {} as any);
...
const poRows = computed<any[]>(() => (poAnggarans.value?.data || []) as any[]);
const poPage = computed<any | null>(() => poAnggarans.value || null);
...
function handleMobileAction(action: 'detail' | 'download' | 'log' | 'edit', row: any) {
  ...
  if (action === 'log') {
    router.visit(`/po-anggaran/${row.id}/log`);
    return;
  }
  if (action === 'edit') {
    if (isCreatorRow(row) || isAdmin.value) {
      router.visit(`/po-anggaran/${row.id}/edit`);
    } else {
      router.visit(`/po-anggaran/${row.id}`);
    }
  }
