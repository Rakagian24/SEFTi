...
function initializeFormData(edit: EditData, po?: PurchaseOrder) {
  // Prefer outstanding from PO if available, so edit form reflects remaining amount
  const baseOutstanding = po && typeof (po as any).outstanding === "number"
    ? Number((po as any).outstanding)
    : null;

  const initialNominal = baseOutstanding !== null
    ? baseOutstanding
    : (edit.total ?? Number(po?.grand_total ?? po?.total ?? 0));

  form.value = {
    no_mb: edit.no_mb || "",
    tanggal: edit.tanggal || "",
    purchase_order_id: String(edit.purchase_order_id || po?.id || ""),
    nominal: formatCurrency(initialNominal || 0),
