...
<template>
  <EmptyState
    ...
    description="Belum ada data. Tambah Jenis Barang pertama Anda."
    icon="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"
    action-text="Add Jenis Barang"
    :show-action="true"
    @action="handleAdd"
  />
    ...
    <div class="overflow-x-auto rounded-lg">
      <table class="min-w-full">
        <thead class="bg-[#FFFFFF] border-b border-gray-200">
          <tr>
            <th class="px-6 py-4 text-left text-xs font-bold text-[#101010] uppercase tracking-wider whitespace-nowrap">Nama Jenis Barang</th>
            <th class="px-6 py-4 text-left text-xs font-bold text-[#101010] uppercase tracking-wider whitespace-nowrap">Singkatan</th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="row in items?.data" :key="row.id" class="alternating-row">
            <td class="px-6 py-4 whitespace-nowrap text-sm [#101010]">{{ row.nama_jenis_barang }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm [#101010]">{{ row.singkatan }}</td>
