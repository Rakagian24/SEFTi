...
const props = defineProps<{ editData?: Record<string, any> }>();
...
function validate() {
  errors.value = {};
  if (!form.value.nama_jenis_barang) errors.value.nama_jenis_barang = "Nama Jenis Barang wajib diisi.";
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      form.value = { nama_jenis_barang: "", singkatan: "", status: "active" };
    }
  },
  { immediate: true }
);
...
function submit() {
  ...
  clearAll();
  if (props.editData) {
    router.put(`/jenis-barangs/${props.editData.id}`, form.value, {
      onSuccess: () => {
        addSuccess("Data Jenis Barang berhasil diperbarui");
        emit("close");
    ...
    router.post("/jenis-barangs", form.value, {
      onSuccess: () => {
        addSuccess("Data Jenis Barang berhasil ditambahkan");
        emit("close");
        window.dispatchEvent(new CustomEvent("table-changed"));
      },
      onError: (serverErrors: any) => {
    ...
    <div class="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl">
      <div class="p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-semibold text-gray-800">{{ props.editData ? "Edit Jenis Barang" : "Tambah Jenis Barang" }}</h2>
        ...
        <form @submit.prevent="submit" novalidate class="space-y-4">
          <div class="floating-input">
            <input v-model="form.nama_jenis_barang" :class="{'border-red-500': errors.nama_jenis_barang}" type="text" id="nama_jenis_barang" class="floating-input-field" placeholder=" " required />
            <label for="nama_jenis_barang" class="floating-label">Nama Jenis Barang<span class="text-red-500">*</span></label>
            <div v-if="errors.nama_jenis_barang" class="text-red-500 text-xs mt-1">{{ errors.nama_jenis_barang }}</div>
          </div>

          <div class="floating-input">
            <input v-model="form.singkatan" :class="{'border-red-500': errors.singkatan}" type="text" id="singkatan" class="floating-input-field" placeholder=" " />
            <label for="singkatan" class="floating-label">Singkatan</label>
