  ...
  <div class="bg-white rounded-b-lg shadow-b-sm border-b border-gray-200">
    ...
    >
      <nav class="flex items-center space-x-2" aria-label="Pagination">
        <!-- Previous Button -->
        <button
          @click="goToPage(pphs?.prev_page_url)"
          :disabled="!pphs?.prev_page_url"
          :class="[
        ...
        <!-- Page Numbers -->
        <template
          v-for="(link, index) in pphs?.links?.slice(1, -1)"
          :key="index"
        >
          <button
        ...
        <!-- Next Button -->
        <button
          @click="goToPage(pphs?.next_page_url)"
          :disabled="!pphs?.next_page_url"
          :class="[
            'px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200',
            pphs?.next_page_url
              ? 'text-gray-600 hover:text-gray-800 hover:bg-gray-50'
