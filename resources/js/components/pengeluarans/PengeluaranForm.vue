...
function submit() {
  ...
  if (props.editData) {
    router.put(`/pengeluarans/${props.editData.id}`, form.value, {
      onSuccess: () => {
        addSuccess('Data pengeluaran berhasil diperbarui');
        emit("close");
        // Dispatch event untuk memberitahu sidebar bahwa ada perubahan
        window.dispatchEvent(new CustomEvent("table-changed"));
      },
      onError: () => {
        addError('Gagal memperbarui data pengeluaran');
      }
    });
  } else {
    router.post("/pengeluarans", form.value, {
      onSuccess: () => {
        addSuccess('Data pengeluaran berhasil ditambahkan');
        emit("close");
        // Dispatch event untuk memberitahu sidebar bahwa ada perubahan
        window.dispatchEvent(new CustomEvent("table-changed"));
      },
      onError: () => {
        addError('Gagal menambahkan data pengeluaran');
      }
    });
  }
}
      ...
      <div class="p-6">
        ...
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-semibold text-gray-800">
            {{ props.editData ? "Edit Pengeluaran" : "Tambah Pengeluaran" }}
          </h2>
          <button
            @click="emit('close')"
