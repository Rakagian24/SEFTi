<template>
  <div v-if="terminInfo" class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
    <div class="flex items-center justify-between mb-2">
      ...
      >
        {{
          terminInfo.status_termin === 'completed' ? 'Selesai' :
          terminInfo.status_termin === 'in_progress' ? 'Sedang Berjalan' : 'Belum Dimulai'
        }}
      </span>
    </div>
    <div class="grid grid-cols-2 gap-2 text-blue-700">
      <div>
        ...
        <span class="ml-1">{{ terminInfo.jumlah_termin_dibuat }} dari {{ terminInfo.jumlah_termin }}</span>
      </div>
      <div>
      ...
      <div v-if="terminInfo.sisa_pembayaran > 0" class="col-span-2">
        ...
        <span class="ml-1 text-orange-600 font-semibold">{{ formatCurrency(terminInfo.sisa_pembayaran) }}</span>
      </div>
    </div>
  </div>
</template>
