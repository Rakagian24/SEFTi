<template>
  <div class="bg-white rounded-t-lg shadow-sm border-t border-gray-200 relative z-50">
    <div class="px-6 py-4">
      <div class="flex gap-4 flex-wrap justify-between">
        <!-- LEFT: Filter toggle & dropdowns -->
        <div class="flex flex-col self-end gap-0 flex-1 min-w-0">
          <Transition name="filter-expand">
            <div
              v-if="isFilterOpen"
              class="mb-3 flex flex-wrap items-center gap-x-4 gap-y-2 max-w-full pb-4"
            >
              <!-- Department -->
              <div class="flex-shrink-0">
                <CustomSelectFilter
                  :model-value="(localFilters.department_id ?? '').toString()"
                  @update:modelValue="(v: string) => { localFilters.department_id = v; applyFilter(); }"
                  :options="departmentOptionsMapped"
                  style="min-width: 14rem"
                />
              </div>

              <!-- Item / Barang -->
              <div class="flex-shrink-0">
                <CustomSelectFilter
                  :model-value="localFilters.item_name"
                  @update:modelValue="(v: string) => { localFilters.item_name = v; applyFilter(); }"
                  :options="itemOptions"
                  :searchable="true"
                  placeholder="Pilih Barang"
                  style="min-width: 14rem"
                />
              </div>

              <!-- Date Range -->
              <div class="flex-shrink-0">
                <DateRangeFilter
                  :start="localFilters.tanggal_start"
                  :end="localFilters.tanggal_end"
                  @update:start="(v: string) => { localFilters.tanggal_start = v; applyFilter(); }"
                  @update:end="(v: string) => { localFilters.tanggal_end = v; applyFilter(); }"
                />
              </div>

              <!-- Reset -->
              <button
                @click="resetFilter"
                class="flex-shrink-0 rounded hover:bg-gray-100 text-gray-400 hover:text-red-500 transition-colors duration-150 mt-1"
                title="Reset filter"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
                  stroke="currentColor"
                  class="size-6"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                  />
                </svg>
              </button>
            </div>
          </Transition>

          <!-- Toggle button -->
          <div class="flex items-center cursor-pointer select-none" @click="toggleFilter">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="size-6"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 3c2.755 0 5.455.232 8.083.678.533.09.917.556.917 1.096v1.044a2.25 2.25 0 0 1-.659 1.591l-5.432 5.432a2.25 2.25 0 0 0-.659 1.591v2.927a2.25 2.25 0 0 1-1.244 2.013L9.75 21v-6.568a2.25 2.25 0 0 0-.659-1.591L3.659 7.409A2.25 2.25 0 0 1 3 5.818V4.774c0-.54.384-1.006.917-1.096A48.32 48.32 0 0 1 12 3Z"
              />
            </svg>
            <span
              :class="'inline-block transition-transform duration-300 ml-2 ' + (isFilterOpen ? 'rotate-45' : 'rotate-0')"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="2"
                stroke="currentColor"
                class="w-4 h-4 text-gray-600"
              >
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
              </svg>
            </span>
            <span class="ml-2 text-gray-700 text-sm font-medium">Filter</span>
          </div>
        </div>

        <!-- RIGHT: show entries + search -->
        <div class="flex items-end gap-4 flex-wrap flex-shrink-0 mt-4">
          <div class="flex items-center text-sm text-gray-700">
            <span class="mr-2">Show</span>
            <div class="relative">
              <CustomSelectFilter
                v-model="localPerPage"
                :options="[
                  { label: '10', value: '10' },
                  { label: '25', value: '25' },
                  { label: '50', value: '50' },
                  { label: '100', value: '100' },
                ]"
                width="5.5rem"
                @update:modelValue="updatePerPage"
              />
            </div>
            <span class="ml-2">entries</span>
          </div>

          <div class="relative flex-1 min-w-64">
            <input
              v-model="localSearch"
              @input="onSearchInput"
              type="text"
              placeholder="Search..."
              class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#5856D6] focus:border-transparent text-sm"
            />
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <svg
                class="h-4 w-4 text-gray-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
                />
              </svg>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { computed, ref } from 'vue';
import CustomSelectFilter from '@/components/ui/CustomSelectFilter.vue';
import DateRangeFilter from '@/components/ui/DateRangeFilter.vue';

const props = defineProps<{
  departments: any[];
  itemOptions: { label: string; value: string }[];
  loading?: boolean;
  filters: {
    department_id: string | number | null;
    item_name: string;
    tanggal_start: string;
    tanggal_end: string;
    per_page: number | string;
    search: string;
  };
}>();

const emit = defineEmits<{
  filter: [payload: any];
  reset: [];
  'update:per-page': [value: number];
  'update:search': [value: string];
}>();

const isFilterOpen = ref(true);

const localFilters = ref({
  department_id: (props.filters.department_id ?? '').toString(),
  item_name: props.filters.item_name || '',
  tanggal_start: props.filters.tanggal_start || '',
  tanggal_end: props.filters.tanggal_end || '',
});

const localPerPage = ref((props.filters.per_page ?? 10).toString());
const localSearch = ref(props.filters.search || '');

const departmentOptionsMapped = computed(() => {
  const base = [{ label: 'Departemen', value: '' }];
  const opts = (props.departments || []).map((d: any) => ({
    label: d.label || d.name,
    value: (d.value ?? d.id).toString(),
  }));
  return [...base, ...opts];
});

function toggleFilter() {
  isFilterOpen.value = !isFilterOpen.value;
}

function applyFilter() {
  const payload: Record<string, any> = {
    department_id: localFilters.value.department_id || '',
    item_name: localFilters.value.item_name || '',
    tanggal_start: localFilters.value.tanggal_start || '',
    tanggal_end: localFilters.value.tanggal_end || '',
  };
  emit('filter', payload);
  window.dispatchEvent(new CustomEvent('content-changed'));
}

function resetFilter() {
  localFilters.value = {
    department_id: '',
    item_name: '',
    tanggal_start: '',
    tanggal_end: '',
  };
  localPerPage.value = '10';
  localSearch.value = '';
  emit('reset');
  emit('update:per-page', 10);
  emit('update:search', '');
  window.dispatchEvent(new CustomEvent('content-changed'));
}

function updatePerPage(v: string) {
  localPerPage.value = v;
  const num = Number(v) || 10;
  emit('update:per-page', num);
  applyFilter();
}

let searchTimer: number | null = null;
function onSearchInput() {
  if (searchTimer) {
    clearTimeout(searchTimer);
  }
  searchTimer = window.setTimeout(() => {
    emit('update:search', localSearch.value || '');
  }, 300);
}
</script>

<style scoped>
.rotate-45 {
  transform: rotate(45deg);
}
.rotate-0 {
  transform: rotate(0deg);
}

.filter-expand-enter-active,
.filter-expand-leave-active {
  transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
  overflow: hidden;
}
.filter-expand-enter-from,
.filter-expand-leave-to {
  opacity: 0;
  max-height: 0;
  margin-bottom: 0;
  padding-bottom: 0;
}
.filter-expand-enter-to,
.filter-expand-leave-from {
  opacity: 1;
  max-height: 300px;
  margin-bottom: 0.75rem;
  padding-bottom: 1rem;
}
</style>
