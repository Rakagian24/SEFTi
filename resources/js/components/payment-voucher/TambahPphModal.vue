...
const emit = defineEmits(["update:open", "add-pph"]);
...
const displayTarif = computed<string>({
  get: () => formatCurrency(form.value.tarif ?? ""),
  set: (val: string) => {
    const parsed = parseCurrency(val);
    form.value.tarif = parsed === "" ? null : Number(parsed);
  },
});
...
function close() {
  ...
  form.value = { kode: "", nama: "", tarif: null, deskripsi: "" };
  errors.value = {};
}
...
function addPph() {
  ...
  emit("add-pph", {
    ...
    tarif: form.value.tarif ? form.value.tarif / 100 : 0, // Convert to decimal
...
<template>
  ...
  >
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl relative">
      ...
      <!-- Body -->
      <div class="px-6 py-6">
        <form @submit.prevent="addPph" class="space-y-5">
          <!-- Kode PPH -->
          ...
          <div
            v-if="errors.general"
            class="text-red-500 text-sm bg-red-50 p-3 rounded-md border border-red-200"
          >
            {{ errors.general }}
          </div>
        </form>
