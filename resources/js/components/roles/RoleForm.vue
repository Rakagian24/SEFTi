...
function validate() {
  errors.value = {};
  if (!form.value.name) errors.value.name = "Nama peran wajib diisi";
...
function submit() {
  ...
  clearAll();
  if (props.editData) {
    router.put(`/roles/${props.editData.id}`, form.value, {
      onSuccess: () => {
      ...
      onError: (serverErrors) => {
        ...
        if (serverErrors && typeof serverErrors === "object") {
          ...
          addError(messages || "Gagal memperbarui role");
        } else {
          addError("Gagal memperbarui role");
        }
      },
    });
  } else {
    router.post("/roles", form.value, {
      onSuccess: () => {
        addSuccess("Role berhasil ditambahkan");
        emit("close");
        window.dispatchEvent(new CustomEvent("table-changed"));
      },
      onError: (serverErrors) => {
        ...
        if (serverErrors && typeof serverErrors === "object") {
          ...
          const messages = Object.values(serverErrors).flat().join(" ");
          addError(messages || "Gagal menambahkan role");
        } else {
          addError("Gagal menambahkan role");
        }
      },
    });
  }
      ...
      <div class="p-6">
        ...
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-semibold text-gray-800">
            {{ props.editData ? "Edit Role" : "Tambah Role" }}
          </h2>
          <button
            @click="emit('close')"
