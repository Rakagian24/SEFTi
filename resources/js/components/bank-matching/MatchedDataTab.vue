...
async function loadMatchedData(page = 1) {
  loading.value = true;
  error.value = '';
  ...
  try {
    ...
    matchedData.value = response.data.data || [];
    pagination.value = {
    ...
    error.value = error.response?.data?.message || 'Error loading matched data: ' + error.message;
    matchedData.value = [];
  } finally {
    loading.value = false;
  }
  ...
  <div class="space-y-4">
    ...
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-4">
      <div class="flex">
        ...
        <div class="ml-3">
          <h3 class="text-sm font-medium text-red-800">Error loading matched data</h3>
    ...
    <!-- Empty State -->
    <div v-else-if="!matchedData.length" class="text-center py-12">
      <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-gray-100">
        <svg class="h-6 w-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      </div>
      <h3 class="mt-2 text-sm font-medium text-gray-900">Tidak ada data yang sudah dimatch</h3>
